...
get_process_info() {
    local pid=$1
    ps -p $pid -o pid,ppid,user,%cpu,%mem,command 2>/dev/null || echo "进程不存在"
}
...
get_thread_info() {
    local pid=$1
    echo "线程信息 (macOS):"
    ps -p $pid -o pid,ppid,user,%cpu,command 2>/dev/null || echo "无法获取线程信息"
}

# 诊断macOS Java环境
diagnose_macos_java() {
    log_info "=== macOS Java环境诊断 ==="
    ...
    if [[ -d "/Library/Java/JavaVirtualMachines" ]]; then
        echo "   系统JDK目录:"
        ls -la /Library/Java/JavaVirtualMachines/ 2>/dev/null | head -5 || echo "   无法访问"
    else
        echo "   系统JDK目录不存在"
    fi
    ...
    if [[ -d "$HOME/Library/Java/JavaVirtualMachines" ]]; then
        echo "   用户JDK目录:"
        ls -la "$HOME/Library/Java/JavaVirtualMachines/" 2>/dev/null | head -5 || echo "   无法访问"
    else
        echo "   用户JDK目录不存在"
    fi
    ...
    for tool in "${tools[@]}"; do
        local tool_path=$(get_jvm_tool_path "$tool")
        if [[ -n "$tool_path" ]]; then
            echo "   $tool: $tool_path"
        else
            echo "   $tool: 不可用"
...
get_system_info() {
    ...
    echo "内存信息: $(vm_stat 2>/dev/null | head -1 || echo '未知')"
    echo "负载情况: $(uptime)"
    echo "当前时间: $(date)"
    echo ""
...
# 查找Java Home路径 (macOS专用)
find_java_home() {
    if [[ -n "$JAVA_HOME" ]]; then
        echo "$JAVA_HOME"
    elif command -v java &> /dev/null; then
        # 通过java命令查找
        java -XshowSettings:properties -version 2>&1 | grep "java.home" | awk '{print $3}'
    elif [[ -d "/Library/Java/JavaVirtualMachines" ]]; then
        # 查找已安装的JDK
        find /Library/Java/JavaVirtualMachines -name "jstack" -type f 2>/dev/null | head -1 | sed 's|/bin/jstack||'
    else
        echo ""
    fi
}
...
get_jvm_tool_path() {
    ...
    # 查找系统安装的JDK
    local system_tool=$(find /System/Library/Frameworks/JavaVM.framework -name "$tool_name" -type f 2>/dev/null | head -1)
    ...
    # 查找用户安装的JDK
    local user_tool=$(find ~/Library/Java/JavaVirtualMachines -name "$tool_name" -type f 2>/dev/null | head -1)
    if [[ -n "$user_tool" ]]; then
        echo "$user_tool"
        return
    fi
...
analyze_java_tools() {
    ...
    if [[ -z "$jstack_path" ]]; then
        ...
        log_info "查找macOS上的Java安装:"
        if [[ -d "/Library/Java/JavaVirtualMachines" ]]; then
            echo "系统JDK:"
            ls -la /Library/Java/JavaVirtualMachines/ 2>/dev/null || echo "无法访问系统JDK目录"
        fi
        
        if [[ -d "$HOME/Library/Java/JavaVirtualMachines" ]]; then
            echo "用户JDK:"
            ls -la "$HOME/Library/Java/JavaVirtualMachines/" 2>/dev/null || echo "无法访问用户JDK目录"
        fi
    ...
    local cmd1="timeout_macos 30 \"$jstack_path\" $pid > \"$thread_dump_file\" 2>/dev/null"
    log_info "执行命令: $cmd1"
    if timeout_macos 30 "$jstack_path" $pid > "$thread_dump_file" 2>/dev/null; then
    ...
    if [[ "$dump_success" == "false" ]]; then
        log_info "尝试方式2: 使用sudo权限..."
        local cmd2="timeout_macos 30 sudo \"$jstack_path\" $pid > \"$thread_dump_file\" 2>/dev/null"
        log_info "执行命令: $cmd2"
        if timeout_macos 30 sudo "$jstack_path" $pid > "$thread_dump_file" 2>/dev/null; then
            if [[ -s "$thread_dump_file" ]]; then
                dump_success=true
                log_info "线程转储成功 (方式2)"
            fi
    ...
    # 方式3: 使用-F强制模式 (如果前两种方式失败)
    if [[ "$dump_success" == "false" ]]; then
        log_info "尝试方式3: 使用强制模式..."
        local cmd3="timeout_macos 30 \"$jstack_path\" -F $pid > \"$thread_dump_file\" 2>/dev/null"
        log_info "执行命令: $cmd3"
        if timeout_macos 30 "$jstack_path" -F $pid > "$thread_dump_file" 2>/dev/null; then
            if [[ -s "$thread_dump_file" ]]; then
                dump_success=true
                log_info "线程转储成功 (方式3)"
            fi
    ...
    # 方式4: 使用sudo + 强制模式 (最后尝试)
    if [[ "$dump_success" == "false" ]]; then
        log_info "尝试方式4: 使用sudo + 强制模式..."
        local cmd4="timeout_macos 30 sudo \"$jstack_path\" -F $pid > \"$thread_dump_file\" 2>/dev/null"
        log_info "执行命令: $cmd4"
        if timeout_macos 30 sudo "$jstack_path" -F $pid > "$thread_dump_file" 2>/dev/null; then
            if [[ -s "$thread_dump_file" ]]; then
                dump_success=true
                log_info "线程转储成功 (方式4)"
            fi
    ...
    if [[ "$dump_success" == "true" ]]; then
        ...
        # 显示进程详细信息
        log_info "进程详细信息:"
        ps -p $pid -o pid,ppid,user,%cpu,%mem,command 2>/dev/null || echo "无法获取进程信息"
    fi
